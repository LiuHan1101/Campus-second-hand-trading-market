...
Page({
    ...
    onLoad(options) {
      ...
      if (!wx.cloud) {
        ...
        wx.cloud.init({
          env: 'cloud1-8gw6xrycfea6d00b', // 替换为您的云环境ID
          traceUser: true,
        });
      }
      const id = options.id;
  ...
  if (userInfo && userInfo.openid) {
    this.setData({ currentUserOpenid: userInfo.openid });
  }
  ...
  if (goodsData) {
    ...
    async checkFavoriteStatus(goodsId) {
      try {
        ...
        this.setData({ isFavorite: result.total > 0 });
      } catch (error) {
  ...
  //切换收藏方法
    async onToggleFavorite() {
      const { goods, isFavorite, currentUserOpenid } = this.data;
      ...
      try {
        ...
        if (isFavorite) {
          // 取消收藏
          await this.removeFavoriteFromDB(goods.id);
        } else {
          // 添加收藏
          await this.addFavoriteToDB(goods.id);
        }
      } catch (error) {
...
// 添加到数据库收藏
// 修改 addFavoriteToDB 方法，添加错误处理
async addFavoriteToDB(goodsId) {
  try {
    const db = wx.cloud.database();
    const { currentUserOpenid, goods } = this.data;
    ...
    const addRes = await db.collection('favorites').add({
      data: {
        ...
        postTitle: goods.title,
        postImage: goods.images && goods.images.length > 0 ? goods.images[0] : '/images/default.jpg',
        postPrice: goods.price,
        transactionType: goods.transactionType,
        postDescription: goods.description ? goods.description.substring(0, 50) + '...' : ''
      }
    });
    
    console.log('收藏添加成功:', addRes);
    ...
    try {
      ...
      console.log('收藏计数更新成功');
    } catch (updateError) {
      console.warn('更新收藏计数失败，但收藏已添加:', updateError);
      // 忽略计数更新失败
    }
    ...
    console.error('添加收藏失败，详细信息:', error);
    
    // 更详细的错误处理
    if (error.errCode === -502005) {
      wx.showModal({
        title: '数据库错误',
        content: '收藏功能需要配置数据库，请联系管理员',
        showCancel: false
      });
    } else if (error.errCode === -501003) {
      wx.showToast({
        title: '网络异常，请重试',
        icon: 'none'
      });
...
// 从数据库移除收藏
async removeFavoriteFromDB(goodsId) {
  try {
    const db = wx.cloud.database();
    const { currentUserOpenid } = this.data;
    ...
    try {
      ...
      console.error('更新收藏计数失败:', updateError);
    }
