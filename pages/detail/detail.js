...
Page({
  ...
  // 弹窗确认发送气泡事件 - 跳转到聊天页面
  onPopupConfirmBubble(e) {
    const { transactionInfo, goodsId, sellerInfo } = e.detail;
    console.log('收到发送气泡事件:', transactionInfo);
    ...
    // 直接跳转到聊天页面，携带交易信息
    wx.navigateTo({
      url: `/pages/chatdetail/chatdetail?goodsId=${goodsId}&sellerInfo=${encodeURIComponent(JSON.stringify(sellerInfo))}&transactionInfo=${encodeURIComponent(JSON.stringify(transactionInfo))}`
    });
  },
